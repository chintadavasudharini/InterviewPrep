...
// The topic is python-polymorphism
// I'll find its sections and replace the end part.

const startId = content.indexOf('"id": "python-polymorphism"');
const sectionsStart = content.indexOf('"sections": [', startId);
const sectionsEnd = content.indexOf('],', sectionsStart) + 1; // Closing ] of sections array

let sectionsStr = content.slice(sectionsStart, sectionsEnd);

// I'll replace the part from ". What is Polymorphism?" onwards with the new content.
const questionsStart = sectionsStr.indexOf('". What is Polymorphism?"');
if (questionsStart !== -1) {
    // Find the opening { of that section
    const openingBrace = sectionsStr.lastIndexOf('{', questionsStart);
    ...
    const updatedSectionsStr = sectionsStr.slice(0, openingBrace).trimEnd() + ',' + newContent;
    content = content.slice(0, sectionsStart) + updatedSectionsStr + content.slice(sectionsEnd);
    fs.writeFileSync(path, content);
    console.log('Successfully updated polymorphism questions and table.');
} else {
    console.log('Could not find the insertion point in polymorphism topic.');
}
